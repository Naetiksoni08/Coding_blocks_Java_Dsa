	...
	class Main {
		static class BinaryTree {
			...
			private Info largestBSTinBT(Node root) {
				...
				Info lefttree = largestBSTinBT(root.left);
				Info righttree = largestBSTinBT(root.right);

				Info self = new Info();

				if (lefttree.isBST && righttree.isBST && root.data > lefttree.max && root.data < righttree.min) {
					// first of all the leftree and righttree themselves be a bst then the left tree
					// ka max should be less than the value of the root as if not then it doesnt
					// satisfy the properties of the bst then the right tree min value should be
					// greater than the root data
					self.size = lefttree.size + righttree.size + 1;
					self.min = Math.min(root.data, lefttree.min);
					self.max = Math.max(root.data, righttree.max);
					self.ans = self.size;
					self.isBST = true;
					return self;

				}
				self.ans = Math.max(lefttree.ans, righttree.ans);
				self.isBST = false;
				return self;
