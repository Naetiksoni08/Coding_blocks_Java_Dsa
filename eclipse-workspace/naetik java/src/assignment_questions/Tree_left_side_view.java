...
public class Tree_left_side_view {
	...
	static class BinaryTree {
		...
		private void leftSideViewResult(Node root, int currentlevel) {
			if (root == null) {
				return;
			}
			if (currentlevel > maxd && root.data != -1) {
				System.out.print(root.data + " ");
				maxd = currentlevel;
			}
			leftSideViewResult(root.left, currentlevel + 1);
			leftSideViewResult(root.right, currentlevel + 1);
